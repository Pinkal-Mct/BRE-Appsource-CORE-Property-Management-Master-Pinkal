page 73209702 "BLRMergeSameSqureSubPage"
{
    PageType = ListPart;
    SourceTable = "BLRMergeSameSqureSubPage";
    ApplicationArea = All;
        ...
        area(content)
        {
            repeater(Group)
            {
                ...
                field("MS_Rate per Sq.Ft"; rec."BLRMS_Rate per Sq.Ft")
                {
                    ...
                    trigger OnValidate()
                    var
                        PreviousYearRecord: Record "BLRMergeSameSqureSubPage";
    ...
    actions
    {
        area(processing)
        {
            action(InsertData)
            {
                ...
                trigger OnAction()
                var
                    MergeSameSquareRec: Record "BLRMergeSameSqureSubPage";
                ...
                begin
                    ...
                    MergeSameSquareRec.SetRange("BLRProposal ID", Rec."BLRProposal ID");
                        ...
                        repeat
                            ...
                            if SubLeaseMergeRec.FindSet() then
                                repeat
                                    ...
                                    PerDayRevnue."BLRProposal ID" := MergeSameSquareRec."BLRProposal ID";
                                    ...
                                    if MergeSameSquareRec."BLRMS_Unit Sq Ft" = 0 then
                                        Error('MS Unit Sq Ft cannot be zero for Proposal ID %1.', MergeSameSquareRec."BLRProposal ID");
    ...
    var
        ...
        MergeSameSquareSubPage: Record "BLRMergeSameSqureSubPage";
    ...
    begin
        ...
        MergeSameSquareSubPage.SetRange("BLRProposal ID", Rec."BLRProposal ID");

        // Sum up the values for the filtered records
        if MergeSameSquareSubPage.FindSet() then
            repeat
                lTotalFinalAmount += MergeSameSquareSubPage."BLRMS_Final Annual Amount";

                // Check for the first year and assign its Final Annual Amount
                if MergeSameSquareSubPage."BLRMS_Year" = 1 then
                    FirstYearAnnualAmount := MergeSameSquareSubPage."BLRMS_Final Annual Amount";
    ...
    begin
        ...
        DaysInPeriod := Rec."BLRMS_End Date" - Rec."BLRMS_Start Date" + 1;

        DaysInYear := 365;

        if IsLeapYear(Date2DMY(Rec."BLRMS_Start Date", 3)) then begin
            LeapDay := DMY2Date(29, 2, Date2DMY(Rec."BLRMS_Start Date", 3));

            if (LeapDay >= Rec."BLRMS_Start Date") and (LeapDay <= Rec."BLRMS_End Date") then
                DaysInYear := 366;
        end;
