page 73209733 "BLRSingleUnitRentSubPage"
{
    PageType = ListPart;
    SourceTable = "BLRSingleUnitRentSubPage";
    ApplicationArea = All;
        ...
        area(content)
        {
            repeater(Group)
            {
                ...
                field("Rate per Sq.Ft"; rec."BLRRate per Sq.Ft")
                {
                    ...
                    trigger OnValidate()
                    var
                        PreviousYearRecord: Record "BLRSingleUnitRentSubPage";
    ...
    var
        ...
        RecordTemp: Record "BLRSingleUnitRentSubPage";
    ...
    begin
        ...
        DaysInPeriod := Rec."BLREnd Date" - Rec."BLRStart Date" + 1;

        DaysInYear := 365;

        if IsLeapYear(Date2DMY(Rec."BLRStart Date", 3)) then begin
            LeapDay := DMY2Date(29, 2, Date2DMY(Rec."BLRStart Date", 3));

            if (LeapDay >= Rec."BLRStart Date") and (LeapDay <= Rec."BLREnd Date") then
                DaysInYear := 366;
        end;
